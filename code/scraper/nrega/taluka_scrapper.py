...
def valueExtraction:
    #since the district pages are going to be extracted from the database
    #hence here the code block will run till the end of the recordset

    # while districtRecordSet.EOF<> null
    ...
    #url="http://164.100.112.66/netnrega/citizen_html/phy_fin_reptemp.aspx?lflag=eng&state_name=MAHARASHTRA&fin_year=2011-2012&district_code=1809&district_name=AHMEDNAGAR&page=d"
	...
	query="SELECT DistrictUniqueId, Link FROM rega_enrega_districtexpenses"
	...
	for row in url
		...
		#first row of the required data for taluka
		...
		while data_row.td.nextSibling.nextSibling.next.string<>'uTotal':
			...
			#extracting the url, talCode, talName via the href tag
			...
			#taluka code index and value. Block code is 7 characters
			...
			DistrictUniqueId= url[0]
			query="INSERT INTO rega_enrega_blocks " +\
				"(UniqueId, DistrictUniqueId, BlockName_Mr, CreatedOn) values ('%s', '%s', '%s', '%s')"%(code, DistrictUniqueId, name, time)
			...
			query="INSERT INTO rega_enrega_blockexpenses " +\
				"(UniqueId, DistrictUniqueId, NoOfWorks, LabourExpenditures, MaterialExpenditures, CreatedOn, Link)" +\
				"values ('%s', '%s', '%s', '%s', '%s', '%s', '%s')"%(code, DistrictUniqueId, noWorks, labExpn, matExpn, time, url)
			...
			#Inserting the values into the database table for Block_Expenses
			#database code block here
